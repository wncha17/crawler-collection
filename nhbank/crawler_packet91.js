...
// formatToNhStyle도 배열을 받도록 수정
// function formatToNhStyle(encryptedBlocks) {
//     if (!encryptedBlocks || encryptedBlocks.length === 0) return "";

//     const processedBlocks = encryptedBlocks.map(block => {
//         const bytes = block.match(/.{1,2}/g);
...
async function getEncryptedField(fieldName, value, uuid, sessionKey) {
    ...
    const encryptedPacket = "+" + blocks.join(' ');
